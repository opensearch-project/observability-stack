...
<PlaygroundLink query="" text="Try PPL now in the live playground →" />
...
<PlaygroundLink query="%7C%20where%20severityNumber%20%3E%3D%2017%20%7C%20stats%20count%28%29%20as%20errors%20by%20%60resource.attributes.service.name%60%20%7C%20sort%20-%20errors" />
...
The fastest way to learn PPL is to use it. Open the <PlaygroundLink query="" text="live playground" /> and try these queries against live OpenTelemetry data:

**Count logs by service:**
```sql
| stats count() as log_count by `resource.attributes.service.name`
```
<PlaygroundLink query="%7C%20stats%20count%28%29%20as%20log_count%20by%20%60resource.attributes.service.name%60" />

**Find error logs:**
```sql
| where severityText = 'ERROR' or severityText = 'FATAL'
```
<PlaygroundLink query="%7C%20where%20severityText%20%3D%20!%27ERROR!%27%20or%20severityText%20%3D%20!%27FATAL!%27" />
